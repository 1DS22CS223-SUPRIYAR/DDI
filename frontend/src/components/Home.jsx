...
function Home() {
  ...
  const handlePredict = async () => {
    try {
      if (!selectedDrugs.drug1.trim() || !selectedDrugs.drug2.trim()) {
        alert("Please select both drugs");
        return;
      }
      const response = await fetch(
        `http://127.0.0.1:8000/predict_interaction?drug1=${selectedDrugs.drug1}&drug2=${selectedDrugs.drug2}`
      );
      if (!response.ok) {
    ...
    <div>
      ...
      <div className="main-content">
        <div className="container">
          ...
          >
            <p style={{fontSize: 18}}>Predict Interaction</p>
            <p style={{fontSize: 10}}>
            {!selectedDrugs.drug1 || !selectedDrugs.drug2
              ? "(Select both the drugs to predict)"
              : ""}
            </p>
          </button>
